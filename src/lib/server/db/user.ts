...
export async function exists(identifier: string): Promise<boolean> {
	...
	const user_data = await collection.findOne<UserType>(
		{
			$or: [{ token: identifier }, { username: identifier }]
		},
		{ projection: { _id: 1 } }
	);
 ...
 */
export async function find_matching<T extends Partial<UserType>>(
	query: string,
	fields: (keyof T)[]
): Promise<T[]> {
 ...
 */
export async function get_from<T = string>(
	identifier: string,
	...
	const user_data = await collection.findOne<UserType>(
		{
			$or: [{ username: identifier }, { token: identifier }]
		},
		{ projection }
	);
